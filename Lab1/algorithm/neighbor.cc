...
//    __mutex_slove.lock();
  ...
  for (int row = 0; row < ROW; row += 3) {
    for (int col = 0; col < COL; col += 3) {
      int occurs[10] = { 0 };
      ++occurs[chessW[row  ][col]];
      ++occurs[chessW[row  ][col+1]];
      ++occurs[chessW[row  ][col+2]];
      ++occurs[chessW[row+1][col]];
      ++occurs[chessW[row+1][col+1]];
      ++occurs[chessW[row+1][col+2]];
      ++occurs[chessW[row+2][col]];
      ++occurs[chessW[row+2][col+1]];
      ++occurs[chessW[row+2][col+2]];

      if (std::count(occurs, occurs+10, 1) != NUM)
        return false;
    }
  }
//    __mutex_slove.unlock();
  return true;
}
